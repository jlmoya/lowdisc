...
Sobol::Sobol ( int dim_num, int coordinate)
{
	...
	long long int m;
	long long int newv;
	long long int poly_init[I8SOBOL_DIM_MAX2] =
	{
...
#undef POLYNOMIAL_V
	...
	for ( i = 0; i < I8SOBOL_DIM_MAX2; i++ )
	{
		i8sobol_poly[i]=poly_init[i];
	}
	//
	i8sobol_dim_num = dim_num;
	//
	...
	for ( i = 1; i < i8sobol_dim_num; i++ )
	{
		...
		//
		for ( j = m; j < i8sobol_maxcol; j++ )
		{
			newv = i8sobol_v[i][j-m];
			l = 1;
	...
	//
	l = 1;
	for ( j = i8sobol_maxcol - 2; 0 <= j; j-- )
	{
		l = 2 * l;
		for ( i = 0; i < i8sobol_dim_num; i++ )
		{
			i8sobol_v[i][j] = i8sobol_v[i][j] * l;
		}
	}
	//
...
void Sobol::next ( long long int *seed, double quasi[ ] )
{
	//
	...
	else if ( *seed == i8sobol_seed_save + 1 )
	{
		// Regular call: next, next, etc...
		// The offset is equal to 1.
		l = i8_bit_lo0 ( *seed );
	}
	else if ( *seed <= i8sobol_seed_save )
	{
		...
		for ( seed_temp = i8sobol_seed_save; seed_temp <= (*seed)-1; seed_temp++ )
		{
	...
	else if ( i8sobol_seed_save+1 < *seed )
	{
		// Move forward in the sequence : the offset is larger than 1.
		for ( seed_temp = i8sobol_seed_save+1; seed_temp <= (*seed)-1; seed_temp++ )
		{
	...
	if ( i8sobol_maxcol < l )
	{
		...
		msg << "  SEED =   " << *seed  << endl;
		msg << "  MAXCOL = " << i8sobol_maxcol << endl;
		lowdisc_error(msg.str());
		return;
	}
	//
