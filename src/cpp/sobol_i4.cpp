...
//
// Parameters for i4_sobol
//
#define I4SOBOL_DIM_MAX2 1111
#define I4SOBOL_LOG_MAX 30
...
static float i4sobol_recipd;
static int i4sobol_seed_save = - 1;
static int i4sobol_v[I4SOBOL_DIM_MAX2][I4SOBOL_LOG_MAX];
...
//
// Parameters
//   dim_num : the number of dimensions
//
{
	int i;
	int j;
	...
	// Put this into a .h to simplify the source code.
	//
#define POLYNOMIAL_V i4sobol_v
#include "sobol_polynomial.h"
#undef POLYNOMIAL_V
	//
	//  Check parameters.
	//
	if ( dim_num < 1 || I4SOBOL_DIM_MAX2 < dim_num )
	{
		ostringstream msg;
		msg << "sobol - i4_sobol_start - Fatal Error" << endl;
		msg << "  The spatial dimension DIM_NUM should satisfy"<<endl;
		msg << "    1 <= DIM_NUM <= " << I4SOBOL_DIM_MAX2 << endl;
		msg << "  But this input value is DIM_NUM = " << dim_num << endl;
		lowdisc_error(msg.str());
		return;
	}

	i4sobol_dim_num = dim_num;
	//
	//  Set ATMOST = 2^LOG_MAX - 1.
	//
	i4sobol_atmost = 0;
	for ( i = 1; i <= I4SOBOL_LOG_MAX; i++ )
	{
		i4sobol_atmost = 2 * i4sobol_atmost + 1;
	}
	//
	//  Find the highest 1 bit in ATMOST (should be I4SOBOL_LOG_MAX).
	//
	i4sobol_maxcol = i4_bit_hi1 ( i4sobol_atmost );
	//
	//  Initialize row 1 of V.
	//
	for ( j = 0; j < i4sobol_maxcol; j++ )
	{
		i4sobol_v[0][j] = 1;
	}
	//
	...
	for ( i = 1; i < i4sobol_dim_num; i++ )
	{
		...
		//
		for ( j = m; j < i4sobol_maxcol; j++ )
		{
			newv = i4sobol_v[i][j-m];
			l = 1;
	...
	//
	l = 1;
	for ( j = i4sobol_maxcol-2; 0 <= j; j-- )
	{
		l = 2 * l;
		for ( i = 0; i < i4sobol_dim_num; i++ )
		{
			i4sobol_v[i][j] = i4sobol_v[i][j] * l;
		}
	}
	//
...
//
{
	...
	else if ( *seed == i4sobol_seed_save + 1 )
	{
		l = i4_bit_lo0 ( *seed );
	}
	else if ( *seed <= i4sobol_seed_save )
	{
		i4sobol_seed_save = 0;
		l = 1;
		for ( i = 0; i < i4sobol_dim_num; i++ )
		...
		for ( seed_temp = i4sobol_seed_save; seed_temp <= (*seed)-1; seed_temp++ )
		{
			...
			for ( i = 0; i < i4sobol_dim_num; i++ )
			{
				i4sobol_lastq[i] = ( i4sobol_lastq[i] ^ i4sobol_v[i][l-1] );
			}
		}
		l = i4_bit_lo0 ( *seed );
	}
	else if ( i4sobol_seed_save+1 < *seed )
	{
		for ( seed_temp = i4sobol_seed_save+1; seed_temp <= (*seed)-1; seed_temp++ )
		{
	...
	{
		ostringstream msg;
		msg << "sobol - i4_sobol - Error" << endl;
		msg << "  The value of SEED seems to be too large" <<endl;
		msg << "  SEED =   " << *seed  << endl;
		msg << "  MAXCOL = " << i4sobol_maxcol << endl;
		lowdisc_error(msg.str());
		return;
	}
	//
	...
	for ( i = 0; i < i4sobol_dim_num; i++ )
	{
		quasi[i] = ( ( float ) i4sobol_lastq[i] ) * i4sobol_recipd;

		i4sobol_lastq[i] = ( i4sobol_lastq[i] ^ i4sobol_v[i][l-1] );
	}

	i4sobol_seed_save = *seed;
	*seed = *seed + 1;
